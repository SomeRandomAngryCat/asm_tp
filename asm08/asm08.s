...
_start:
    ...
    cmp rcx, 2              ; Vérifier s'il y a exactement 2 arguments (programme + 1 nombre)
    ...
    cmp r12, 0              ; Comparer le nombre à 0
    jl error                ; Si < 0, erreur

    ; Calculer la somme des nombres entiers inférieurs au nombre donné
    xor r13, r13            ; Initialiser la somme à 0
    mov r14, 0              ; Initialiser le compteur à 0

sum_loop:
    cmp r14, r12            ; Comparer le compteur avec le nombre donné
    jge sum_done            ; Si compteur >= nombre, terminer

    add r13, r14            ; Ajouter le compteur à la somme
    inc r14                 ; Incrémenter le compteur
    jmp sum_loop            ; Continuer la boucle

sum_done:
    ; Convertir le résultat en chaîne et l'afficher
    mov rax, r13            ; rax = résultat de la somme
